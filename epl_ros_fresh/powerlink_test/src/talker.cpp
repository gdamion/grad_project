...
int main(int argc, char **argv)
{
    ...
    ros::Publisher powerlink_out_pub = n.advertise<powerlink_test::PowerlinkOut>("PowerlinkOut", 1000);
    ros::Publisher status_out_pub = n.advertise<powerlink_test::StatusOut>("StatusOut", 1000);
    ros::Publisher testing_out_pub = n.advertise<powerlink_test::TestingOut>("TestingOut", 1000);
    ...
    powerlink_test::PowerlinkOut powerlink_out_msg;
    powerlink_out_msg.pid = pid;
    powerlink_out_msg.shm_name = powerlink_out.shm_name;

    powerlink_test::StatusOut status_out_msg;
    status_out_msg.pid = pid;
    status_out_msg.shm_name = status_out.shm_name;

    powerlink_test::TestingOut testing_out_msg;
    testing_out_msg.pid = pid;
    testing_out_msg.shm_name = testing_out.shm_name;
...
void oplk_handler(int sig)
{
    ...
    if (tick)
    {
        ...
        get_status_out_struct(status_out.buffer_a);
        get_testing_out_struct(testing_out.buffer_a);
        set_powerlink_in_struct(powerlink_in.buffer_b);
        // set_fallback_in_struct(fallback_in.buffer_b);
        send_sig(SIGUSR1);
    }
    else
    {
        get_powerlink_out_struct(powerlink_out.buffer_b);
        get_status_out_struct(status_out.buffer_b);
        get_testing_out_struct(testing_out.buffer_b);
        set_powerlink_in_struct(powerlink_in.buffer_a);
        // set_fallback_in_struct(fallback_in.buffer_a);
        send_sig(SIGUSR2);
    }
}
